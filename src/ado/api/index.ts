...
async function fetchWithAuth(url: string, options?: RequestInit, retry: number = 0): Promise<any> {
  try {
 ...
 * @throws {Error}
 */
export async function GetWorkItems(config: AdoConfigData, filter: string): Promise<WorkItem[]> {
  const project = ExtractProject(config);
  const { organization } = config;
  const url = `https://dev.azure.com/${organization}/${project}/_apis/wit/workitems?api-version=7.0&$filter=${filter}`
  ...
  // If we get an error (i.e. Work item does not exist at the specified datetime)
  if (json.message) {
    console.error(`Error getting work items (filter: ${filter})`)
    throw new Error(json.message);
  }
 ...
 * @throws {Error} if item does not exist
 */
export async function GetWIQL(config: AdoConfigData, wiql: string): Promise<unknown> {
  const project = ExtractProject(config);
  const { organization } = config;
  const url = `https://dev.azure.com/${organization}/${project}/_apis/wit/wiql?api-version=6.1-preview.2`
  ...
  // If we get an error (i.e. Work item does not exist at the specified datetime)
  if (json.message) {
    console.error(`Error running WIQL query.`)
    throw new Error(json.message);
  }
